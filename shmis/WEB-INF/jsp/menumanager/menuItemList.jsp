...
Ext.onReady(function(){
 	...
 	gridstore = new Ext.data.JsonStore( {
		...
		idProperty : 'id',
		fields : [ 'id','createByTime','modifiedByTime','text','url','description'],
		// 因为跨域，所以使用ScriptTagProxy，在同一个域里边用HttpProxy
		proxy : new Ext.data.HttpProxy({
			url : '/shmis/background/menuitemmanager/data/menuitems'
		})
	});
	//表格选择框
			       ...
			       var rs=Ext.getCmp("menugrid").getSelectionModel().getSelections();
		               if(rs==null||rs.length!=1){
		                    ...
		                    top.Ext.MessageBox.confirm("确认","确认要删除吗",function(btn){    
	                               if(btn=="yes"){
		                           Ext.Ajax.request({
	             			   url:'/shmis/background/menuitemmanager/d/'+rs[0].json.id,
	             			   method:'POST',
	...
	function addorupdate(type){   //0为新增，1为修改
		...
		if(type==0){
	        	title = "新增";
	        	url = "/shmis/background/menuitemmanager/c/menu"; 
		}else{
			record = (function() {
...
// 			alert(record.length);
			if(record == null ){
				   ...
				   var title="修改";
			           var url ="/shmis/background/menuitemmanager/u/menu"; 
		  	}
		}

...
toUrl("GNLIST_New.html");
    ...
    //run.addParam(YT);
    //run.addParam(YT_ID);
    var val=run.run("House","get_area_val");
    var showarea = val.split("~|~");
   

    var html=showarea[0]+"~|~0~|~170~|~0~|~"+showarea[0]+"~|~DQ.jpg~|~1";
    html+="~@~"+showarea[1]+"~|~0~|~330~|~0~|~"+showarea[1]+"~|~DQ.jpg~|~1";
...
function BindList(val)
{
    ...
    for (var i=0;i<strs.length;i++)
    {
         ...
         if(str[6]==0)
            continue;
			//margin-top:"+(height-130)+";line-height
			//vertical-align:middle;line-height:150;margin-top:"+(height-100)+"
//position:absolute;
//top:50%;left:50%;
//margin:-150px 0 0 -200px;vertical-align:middle;top:50%;left:50%;
//<div position:absolute; style=\"font-weight:bold;text-align:center;color:White;font-family:微软雅黑;\">"+str[0]+"</div>

//original
var obj = $("<div style=\"cursor:pointer;\" onclick=\"gn_click('"+str[0]+"','"+str[1]+"')\" id=\"div"+i+"\"><div style=\"font-weight:bold;text-align:center;margin-top:"+(height-100)+";color:White;font-family:微软雅黑;\">"+str[0]+"</div></div>").appendTo("body");
        //var obj = $("<div style=\"cursor:pointer;\" onclick=\"gn_click('"+str[0]+"','"+str[1]+"')\" id=\"div"+i+"\"><div position:absolute; style=\"font- weight:bold;text-align:center;color:White;font-family:微软雅黑;position:relative; left:50%;top:50%\" ><div style=\"position:relative; left:-50% top:-50%\">"+str[0]+"</div></div></div>").appendTo("body");
...
run.addParam(YT);
run.addParam(YT_ID);
var re = run.run("House","set_area_val");
alert(re);
if(re=="[T]")
{
	toUrl("CXXX.html");

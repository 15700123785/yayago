...
var _word_len = 0;
function chooseCH(str)
{   
    code_field=$("#code_field").html();
    if(str=="+")
    {   
        if (code_field != "") {
				if (start_mem != -1) {
...
//        if(checkid=="edit_area"){
            document.getElementById(checkid).value+=word_list[Number(str)-1];
//        }else
//        {
//            passwordvalue+=word_list[Number(str)-1];
//            document.getElementById(checkid).value=passwordvalue;//replacestr(passwordvalue,"*");
//        }
        $("#code_field").html("");
        $("#list_area").html("");
...
function create_word_list(start, index, str) {
	...
	candidates = "";
	while (cnt <= 10) {
		candidates += "<span onclick='chooseCH("+(cnt % 10==0?10:cnt % 10)+")'>"+(cnt % 10) + '.' + same_code_words[index] + ' </span>&nbsp;&nbsp;';
	...
	if (start > 0) {
		if (start_stack.length > 1) {
			candidates += "<br><span onclick='chooseCH(\"-\")'>上一页</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span onclick='chooseCH(\"+\")'>下一页</span>";
		} else {
			candidates += "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span onclick='chooseCH(\"+\")'>下一页</span>";
		}
	} else if (start_stack.length > 1) {
		candidates +="<br><span onclick='chooseCH(\"-\")'>上一页</span>";
	} else {
		candidates += '';//FIXME';
	}
	start_mem = start;
